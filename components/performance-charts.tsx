...
export default function DataAnalytics({ gameLog, getDistrictName, currentRound }) {
    ...
    <div className="p-4">
      <Tabs defaultValue="population" className="w-full">
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={populationData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Tooltip formatter={(value) => new Intl.NumberFormat().format(value)} />
                  <Legend layout="horizontal" verticalAlign="top" align="center" height={20} />
                  <Line 
                    type="monotone" 
                    dataKey="district1" 
        ...
        <TabsContent value="bias" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Bias Indicators - Fixed to single column */}
            <ChartCard 
            ...
            {/* District Disparity Analysis - Fixed to single column */}
            <ChartCard 
